...
export function AdminProfilePage() {
  ...
  const [form, setForm] = useState({ first_name: '', last_name: '', profile_photo: '' });

  useEffect(() => {
    dispatch(loadCurrentUser());
  }, [dispatch]);

  useEffect(() => { if (user) setForm({ first_name: user.first_name || '', last_name: user.last_name || '', profile_photo: user.profile_photo || '' }); }, [user]);
  ...
  function assetUrl(value: string) {
    ...
    try { const parsed = new URL(value); return parsed.pathname.startsWith('/public/') ? `${apiOrigin}${parsed.pathname}` : value; }
    catch { return value.startsWith('/public/') ? `${apiOrigin}${value}` : value; }
  }
      ...
      <div className='app-shell-inner'>
        ...
        <div className='profile-layout'>
          <aside className='profile-summary card'>
            <div className='avatar'>{form.profile_photo ? <img src={assetUrl(form.profile_photo)} alt="Profile" onError={(event) => { event.currentTarget.style.display = 'none'; }} /> : <span>{(user.first_name || user.email).slice(0, 1).toUpperCase()}</span>}</div>
            <h2>{`${user.first_name ?? ''} ${user.last_name ?? ''}`.trim() || 'Admin'}</h2><p className='muted'>{user.email}</p><span className='badge success'>Admin</span>
          </aside>
